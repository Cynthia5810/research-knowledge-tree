...
概念关系使用受控词表：`is_a`（层级）、`component_of`（组成）、`made_of`（材质）、`has_property`（性质）、`prepared_by`（制备）、`used_for`（应用）、`acts_via`（机制）、`improves`（性能）和 `related_to`（相关）。层级与跨维度逻辑关系分开表达：`is_a` 负责树形递进，其余关系负责把任何概念变成可多向追溯的节点。运行：
...
然后打开 `http://127.0.0.1:4180` 使用本地交互式分类编辑器：

- 按研究对象、材料、方法、机制、性能和应用整理概念。
- 焦点视图：任何概念都可以成为根，按维度（组成、材质、性质、制备、应用、机制、性能）展开正向与反向连接，逐级追溯并保留面包屑路径。
- 使用 `parent` 建立可无限扩展的层级。
- 拖动概念切换类别，在编辑面板中调整上级概念。
- 编辑概念定义和跨分支逻辑关系。
- 将新论文挂载到多个已有概念，或从论文定位界面创建新分支。
- 以关系树检查某个概念的上下游连接。
- 使用 Obsidian Markdown 作为唯一数据源，并保留 JSON 导出备份。

网页与 Obsidian 双向同步：

- 修改 `04_Concept_Nodes/` 后，网页会自动检测并刷新。
- 网页中的新建、分类、父级、定义和关系修改会写回对应 Markdown。
- 网页中的论文定位会写回论文卡的 `concepts` 属性。
- 网页只更新受控 Properties、`定义` 和 `关系` 区块，不覆盖其他研究笔记。
...
- [ ] 增加性能权衡、冲突证据和适用边界表达
- [ ] 把膜科学本体与通用方法框架分离
- [ ] 探索轨迹层：把"提问 → 触达概念 → 命中文献"的人机交互路径记录为可回放的轨迹数据，叠加显示在知识树上（参考 Graphologue / Flowith 的画布交互，但轨迹持久化并与文献锚定）
